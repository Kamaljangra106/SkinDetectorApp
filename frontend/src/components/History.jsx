    ...
    const fetchHistory = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setAnalyses(data.map(normalizeAnalysis))
        }
      } catch {
    ...
    <div className="min-h-[calc(100vh-4rem)]">
      ...
      <div className="bg-gradient-to-r from-teal-500 to-cyan-500 px-6 py-12">
        <div className="max-w-5xl mx-auto">
          ...
          <p className="text-teal-100">Review your past skin analyses and track your progress over time</p>
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-slate-500 max-w-sm">
              Start your first skin analysis to see your history here. Track your skin health journey over time.
            </p>
          </div>
        ) : (
          <div className="space-y-4">
            {analyses.map((analysis, idx) => {
              const severity = SEVERITY_COLORS[analysis.acne_severity] || SEVERITY_COLORS.none
              const itemId = analysis.id || String(idx)
                ...
                >
                  ...
                  >
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-teal-500 to-cyan-500 flex items-center justify-center flex-shrink-0">
                      <span className="text-lg font-bold text-white">{analysis.skin_type.charAt(0)}</span>
                    </div>

                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className="font-semibold text-slate-800">{analysis.skin_type}</span>
                        <span className={`px-2.5 py-0.5 text-xs font-semibold rounded-full border ${severity.text} ${severity.bg} ${severity.border}`}>
                          {analysis.acne_severity.charAt(0).toUpperCase() + analysis.acne_severity.slice(1)}
                        </span>
                      </div>
                      <div className="flex flex-wrap items-center gap-3 text-sm text-slate-500">
                        <span>{formatDate(analysis.created_at)}</span>
                        <span className="w-1 h-1 rounded-full bg-slate-300" />
                        <span>{analysis.fitzpatrick_type}</span>
                        <span className="w-1 h-1 rounded-full bg-slate-300" />
                        <span className="text-teal-600 font-medium">{Math.round(analysis.confidence * 100)}% confidence</span>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="px-6 pb-6 border-t border-slate-100 bg-slate-50/30">
                      <div className="grid gap-6 lg:grid-cols-2 pt-6">
                        {/* Concerns */}
                        <div>
                          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Concerns</h4>
                          <div className="flex flex-wrap gap-2">
                            {analysis.concerns.length > 0 ? (
                              analysis.concerns.map((concern) => (
                                <span key={concern} className="px-3 py-1.5 text-sm font-medium text-slate-600 bg-white rounded-full border border-slate-200">
                                  {concern}
                                </span>
                              ))
                            ) : (
                        ...
                        {/* Ingredients */}
                        <div>
                          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Recommended Ingredients</h4>
                          <div className="space-y-2">
                            {analysis.ingredients.map((item, i) => (
                              <div key={i} className="text-sm text-slate-700 flex items-start gap-2">
                                ...
                                <span>
                                  <span className="font-medium">{item.name}</span> - {item.benefit}
                                </span>
                              </div>
                            ))}
                          </div>
                      ...
                      {analysis.ai_recommendations.length > 0 && (
                        <div className="mt-6">
                          <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-2">
                            <span className="px-1.5 py-0.5 text-[10px] font-bold bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded">AI</span>
                            Personalized Recommendations
                          </h4>
                          <div className="grid gap-3 sm:grid-cols-2">
                            {analysis.ai_recommendations.map((item, i) => (
                              <div key={i} className="p-3 rounded-xl bg-gradient-to-br from-teal-50/50 to-cyan-50/50 border border-teal-100 text-sm">
                                <span className="font-medium text-slate-800">{item.name}</span>
                                <span className="text-slate-500"> - {item.benefit}</span>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {/* Conflicts */}
                      <div className="mt-6">
                        <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Ingredient Conflicts</h4>
                        {analysis.conflicts.length === 0 ? (
                          ...
                          <div className="space-y-2">
                            {analysis.conflicts.map((conflict, i) => (
                              <div key={i} className="p-3 rounded-xl bg-amber-50 border border-amber-100 text-sm">
                                <span className="font-medium text-slate-800">{conflict.pair[0]} + {conflict.pair[1]}</span>
                                <span className="text-slate-600"> - {conflict.reason}</span>
                                <span className="text-amber-700 font-medium"> ({conflict.timing_advice})</span>
                              </div>
                            ))}
                          </div>
                        )}
