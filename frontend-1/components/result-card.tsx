...
export function ResultCard({ result, onAnalyzeAgain }: ResultCardProps) {
  const severityStyle = SEVERITY_COLORS[result.acne_severity];
    ...
    <div className="max-w-6xl mx-auto p-6 lg:p-8 w-full">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-slate-500">
          Here are your personalized skin analysis results
        </p>
      </div>
        ...
        <div className="lg:col-span-1 flex flex-col gap-4">
          ...
          <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden">
            <div className="bg-gradient-to-r from-teal-500 to-cyan-500 px-6 py-6">
              <p className="text-xs font-semibold text-teal-100 uppercase tracking-wider mb-1">
                Your Skin Type
              </p>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <span
                  className={`px-3 py-1.5 text-sm font-semibold rounded-full border ${severityStyle.text} ${severityStyle.bg} ${severityStyle.border}`}
                >
                  {result.acne_severity.charAt(0).toUpperCase() +
                    result.acne_severity.slice(1)}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-semibold text-slate-800">
                  Type {result.fitzpatrick_type}
                </span>
              </div>

              <p className="text-xs text-slate-400 text-right">
                {result.fitzpatrick_label}
              </p>
            ...
            <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {result.concerns.map((concern) => (
                  <span
                    key={concern}
                    className="px-3 py-1.5 text-sm font-medium text-slate-600 bg-slate-100 rounded-full"
                  >
                    {concern}
                  </span>
                ))}
        ...
        <div className="lg:col-span-2 flex flex-col gap-4">
            ...
            <div className="p-6">
              <div className="grid gap-4 sm:grid-cols-2">
                {result.ingredients.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 rounded-xl bg-slate-50 border border-slate-100 hover:border-teal-200 hover:bg-teal-50/50 transition-colors"
                  >
          ...
          <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-teal-100 overflow-hidden">
            <div className="px-6 py-4 bg-gradient-to-r from-teal-50 to-cyan-50 border-b border-teal-100 flex items-center gap-3">
              <div className="px-2 py-1 text-xs font-bold bg-gradient-to-r from-teal-500 to-cyan-500 text-white rounded-md">
                AI
              </div>
              <h3 className="font-semibold text-slate-800">
            ...
            <div className="p-6">
              <div className="grid gap-4 sm:grid-cols-2">
                {result.ai_recommendations.map((item, idx) => (
                  <div
                    key={idx}
                    className="p-4 rounded-xl bg-gradient-to-br from-teal-50/50 to-cyan-50/50 border border-teal-100"
                  >
                    <span className="font-semibold text-slate-800">
                      {item.name}
                    </span>
                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-emerald-50 border border-emerald-100">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-emerald-600">
                      All recommended ingredients are safe to use together
                    </p>
                  </div>
                </div>
              ) : (
