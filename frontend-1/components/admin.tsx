...
interface AdminStats {
  ...
  top_skin_type: string;
  skin_type_breakdown: Array<{ type: string; count: number }>;
}
...
export function Admin({ token }: AdminProps) {
  ...
  const totalBreakdownCount =
    stats?.skin_type_breakdown.reduce((sum, item) => sum + item.count, 0) || 1;
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {statCards.map((card, idx) => (
                <div
                  key={idx}
                  className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 p-6 hover:shadow-xl transition-shadow"
                >
                  <div
                    className={`w-12 h-12 rounded-xl ${card.bgLight} flex items-center justify-center mb-4`}
                  >
                    <div
                  ...
                  <p
                    className={`text-2xl lg:text-3xl font-bold bg-gradient-to-r ${card.color} bg-clip-text text-transparent mt-1`}
                  >
                    {card.value}
                  </p>
                </div>
              ...
              <div className="bg-white rounded-2xl shadow-lg shadow-slate-200/50 border border-slate-100 overflow-hidden">
                ...
                <div className="p-6 space-y-5">
                  {stats?.skin_type_breakdown.map((item, idx) => {
                    const percentage = (item.count / totalBreakdownCount) * 100;
                    const colors = [
